...
diagnosis_cols = ['subject_id', 'hadm_id', 'icd_code', 'icd_version']
d_icd_cols = ['icd_code', 'icd_version', 'long_title']

# Load dataframes
icd_df = pd.read_csv('/Users/maanas/Desktop/Ashoka/ASP/Sem 8/Capstone Thesis/mimic-iv-clinical-database-demo-2.2/hosp/diagnoses_icd.csv', usecols=diagnosis_cols)
d_icd_df = pd.read_csv('/Users/maanas/Desktop/Ashoka/ASP/Sem 8/Capstone Thesis/mimic-iv-clinical-database-demo-2.2/hosp/d_icd_diagnoses.csv', usecols=d_icd_cols)
...
diagnosis_detailed_df = pd.merge(
    sepsis_patients_df, 
    d_icd_df, 
...
print(diagnosis_detailed_df.head())

# Save to CSV
diagnosis_detailed_df.to_csv('New/DIAGNOSES_ICD_LABELED.csv', index=False)

# Print results
print(f"Number of sepsis diagnosis records: {len(diagnosis_detailed_df)}")